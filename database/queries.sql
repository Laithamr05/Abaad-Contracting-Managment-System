-- Abaad Contracting Company - Complex Analytical Queries
-- These queries demonstrate advanced SQL: CTEs, window functions, aggregations

-- ============================================
-- QUERY 1: Project Profitability (Derived)
-- Calculates profit = Revenue - (MaterialCost + LaborCost)
-- ============================================
SELECT 
    p.ProjectID,
    p.ProjectName,
...
LEFT JOIN (
    SELECT ProjectID, SUM(Quantity * UnitPrice) AS TotalMaterialCost
    FROM ProjectMaterial GROUP BY ProjectID
) material_costs ON p.ProjectID = material_costs.ProjectID
LEFT JOIN (
    SELECT wa.ProjectID, SUM(wa.HoursWorked * (e.Salary / 160)) AS TotalLaborCost
    FROM WorkAssignment wa
    INNER JOIN Employee e ON wa.EmployeeID = e.EmployeeID
    GROUP BY wa.ProjectID
) labor_costs ON p.ProjectID = labor_costs.ProjectID
ORDER BY Profit DESC;

-- ============================================
-- QUERY 2: Supplier Impact (Indirect)
-- For each supplier, count distinct projects they indirectly supply
-- ============================================
...
-- ============================================
-- QUERY 3: Hot Materials (Cost Drivers)
-- Materials with highest contribution to costs
-- ============================================
WITH MaterialTotals AS (
    SELECT m.MaterialID, m.MaterialName,
        SUM(pm.Quantity * pm.UnitPrice) AS TotalCost,
...
-- ============================================
-- QUERY 4: Employee Utilization & Management Tree
-- ============================================
...
-- ============================================
-- QUERY 5: Price Anomalies
-- Compare Material.BaseUnitPrice vs SupplierMaterial.Price vs ProjectMaterial.UnitPrice
-- ============================================
...
AnomalyAnalysis AS (
    SELECT pm.ProjectID, p.ProjectName, m.MaterialID, m.MaterialName,
        m.BaseUnitPrice AS BaseUnitPrice,
        COALESCE(smp.MinSupplierPrice, 0) AS MinSupplierPrice,
        COALESCE(smp.AvgSupplierPrice, 0) AS AvgSupplierPrice,
        pm.UnitPrice AS ProjectUnitPrice, pm.Quantity,
...
-- ============================================
-- QUERY 6: Branch Performance Dashboard
-- ============================================
